...
#define MIN(a, b) ((a) < (b) ? (a) : (b))
  ...
  static void __attribute__((noinline)) process_all_task_groups()
  {
    ...
    int local_group_id = warp_id / warps_per_group;                   // which group you are in (1 or 2 ex)
    int group_warp_id = warp_id - local_group_id * warps_per_group;   // warp id of group (if our group uses two warps, you have warp 0 or warp 1)
    int local_task_id = group_warp_id * threads_per_warp + thread_id; // task id inside group
  ...
  void vx_spawn_task_groups(int num_groups, int group_size, vx_spawn_task_groups_cb callback, void *arg)
  {
    ...
    int total_groups_per_core = num_groups / active_cores; // 32 64 groups, 2 core
    ...
    // calculate number of warps to activate
    int groups_per_core = warps_per_core / warps_per_group; // 2 //single invocation
  ...
  void vx_kernel_launch(int grid_x, int grid_y, int grid_z, int block_x, int block_y, int block_z, vx_spawn_task_groups_cb callback, void *arg)
  {
    ...
    if (spatial_mode == 1)
    {
      ...
      vx_spawn_tasks_spatial(grid_x, grid_y, grid_z, block_x, block_y, block_z, callback, arg);
    }
    else
    {
      if (core_id == 0)
      {
        vx_printf("Spatial Mode is OFF\n");
      }
  ...
  /////////////////////////////////////////////////
  // Sapatial
  static void __attribute__((noinline)) process_all_task_spatial()
  {
    wspawn_task_spatial_args_t *targs = (wspawn_task_spatial_args_t *)csr_read(VX_CSR_MSCRATCH);
    ...
    int x_coord = targs->x_coord;
    int y_coord = targs->y_coord;

    // Calculate the number of groups that share the same x and y coordinates
    int total_groups = targs->groups_per_core * vx_num_cores();

    for (int group_id = 0; group_id < total_groups; group_id++)
    {
      // Calculate the x and y coordinates for the current group
      int group_x = group_id % vx_num_cores(); // Assuming cores are arranged in a grid
      int group_y = group_id / vx_num_cores();

      // Check if the group's coordinates match the target coordinates
      if (group_x == x_coord && group_y == y_coord)
      {
        for (int i = 0; i < iterations; i++)
        {
          callback(local_task_id, group_id, local_group_id, warps_per_group, arg);
        }
      }
    }
  }

  static void __attribute__((noinline)) process_all_task_spatial_stub()
  {
    wspawn_task_groups_args_t *targs = (wspawn_task_groups_args_t *)csr_read(VX_CSR_MSCRATCH);
    int warps_per_group = targs->warps_per_group;
    int remaining_mask = targs->remaining_mask;
  ...
  void vx_spawn_tasks_spatial(int grid_x, int grid_y, int grid_z, int block_x, int block_y, int block_z, vx_spawn_task_groups_cb callback, void *arg)
  {
    ...
    // We have only allowed number of cores to have a sqrt that is an integer and is a power of 2
    int cores_per_row = integer_sqrt(num_cores);
    int cores_per_col = cores_per_row;

    int core_row = core_id / cores_per_row;
    int core_col = core_id % cores_per_col;

    // Check if the core is within the active region defined by grid_x and grid_y
    if (core_row >= grid_y || core_col >= grid_x)
    {
    ...
    for (int i = 0; i < num_cores; i++)
    {
      ...
      int col = i % cores_per_col;
      if (row < grid_y && col < grid_x)
      {
        active_cores++;
      }
    }
    ...
    int remaining_threads = group_size - warps_per_group * threads_per_warp;
    int remaining_mask = -1;
    if (remaining_threads != 0) // will be 0 for our basic testing
    {
      remaining_mask = (1 << remaining_threads) - 1;
      warps_per_group++;
    }
    ...
    if (active_warps > warps_per_core)
    {
      active_warps = groups_per_core * warps_per_group; // set it to max warps per core
      warp_batches = total_warps_per_core / active_warps;
      remaining_warps = total_warps_per_core - warp_batches * active_warps; // 0 for our testing
    }

    wspawn_task_spatial_args_t wspawn_args = {
        callback,
        arg,
        core_col,
        core_row,
